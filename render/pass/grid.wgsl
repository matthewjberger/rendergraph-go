...
fn fragment_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let dudv = vec2<f32>(length(dvx), length(dvy));
    let l = length(dudv);
    ...
    let lod0_alpha = max2(vec2<f32>(1.0) - abs(saturate_v2(mod_lod0) * 2.0 - vec2<f32>(1.0)));

    let mod_lod1 = vec2<f32>(
        mod_pos(in.world_pos.x, cell_size_lod1),
        mod_pos(in.world_pos.z, cell_size_lod1)
    ) / dudv4;
    let lod1_alpha = max2(vec2<f32>(1.0) - abs(saturate_v2(mod_lod1) * 2.0 - vec2<f32>(1.0)));

    let mod_lod2 = vec2<f32>(
        mod_pos(in.world_pos.x, cell_size_lod2),
        mod_pos(in.world_pos.z, cell_size_lod2)
    ) / dudv4;
    let lod2_alpha = max2(vec2<f32>(1.0) - abs(saturate_v2(mod_lod2) * 2.0 - vec2<f32>(1.0)));
