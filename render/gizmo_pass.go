...
func NewGizmoPass(device *wgpu.Device, surfaceFormat wgpu.TextureFormat, aspect func() float32) (*Pass, error) {
...
func gizmoPrepare(s any, context *PassContext) error {
	...
	gizmo := *gizmoPtr
	if gizmo == nil || gizmo.Mode == GizmoNone {
		return nil
	}
...
func gizmoExecute(s any, context *PassContext) error {
	state := s.(*gizmoPassState)
	if state.count == 0 {
		return nil
	}
...
func lookupGizmosPtr(world *ecs.World) (**Gizmos, bool) {
