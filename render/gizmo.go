...
type Gizmos struct {
	...
	// Drag state captured in world space at press time. The screen
	// projection is recomputed each frame from these world anchors
	// so the math stays correct if the camera moves mid-drag.
	// initialT is the parametric position of the mouse along the
	// projected axis at press; for rotate, StartWorldVector is the
	// initial offset from the target onto the rotation plane.
